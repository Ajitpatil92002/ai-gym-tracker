...
router.post('/generate-plan', async (req: Request, res: Response) => {
    ...
    try {
        const planSchema = z.object({
            name: z
                .string()
                .describe('A motivating, personalized fitness plan name'),
            description: z
                .string()
                .describe(
                    'A short, engaging description of the plan and what the user will achieve',
                ),
            sessions: z
                .array(
                    z.object({
                        ...
                        description: z
                            .string()
                            .describe(
                                'Brief overview of what this session covers (workout focus + diet theme)',
                            ),
                    }),
                )
                .describe(
                    `Exactly ${daysPerWeek} sessions, one per training day`,
                ),
        });
...
Tone: Energetic, motivating, and professional.`;
    ...
    try {
        ...
        const workoutText =
            dietIdx !== -1 ? fullText.slice(0, dietIdx) : fullText;
        const dietText =
            dietIdx !== -1 && quizIdx !== -1
...
router.put('/sessions/:id/quiz-score', (req: Request, res: Response) => {
    ...
    if (typeof score !== 'number' || typeof isCompleted !== 'boolean') {
        res.status(400).json({
            error: 'score (number) and isCompleted (boolean) are required',
        });
        return;
    }
